...
function chatController($scope, $location, $anchorScroll, socketIO, restService, uSession, usersService) {
      // Initializing users in session.
      // Only these users will be shown in chat window.
      var vm = this;
      $scope.usersInSession = {};
      $scope.rooms = {}; 
      vm.properties=uSession.properties;
      ...
      socketIO.on('userMsgs', function(data){
          ...
          if (first_unread) {
              setTimeout(function(){
                  ...
                  $scope.usersInSession[other].unread = 0;                  // Go to first unread message
      ...
      socketIO.on('openChatUser', function (u) {
          if (u) {
              ...
              uSession.setChatVisibility(true);
      ...
      $scope.msgReceived = function (msg) {
          ...
          if (($scope.userActive == msg.user_src._id.toString() && vm.chat.isShown) || $scope.userActive == msg.user_dst._id.toString()) {
